...
public class CustomRedisTokenStore implements TokenStore {
    ...
    private int getAccessTokenValiditySeconds(String clientId) {
        ...
        byte[] bytes;
        try {
            bytes = conn.get(serializeKey(CommonConstants.CACHE_CLIENT_KEY + ":" + clientId));
        } finally {
            conn.close();
        }
        if (bytes != null) {
        ...
        //返回默认值
        return CommonConstants.ACCESS_TOKEN_VALIDITY_SECONDS;
    }
    ...
    public OAuth2Authentication readAuthentication(String token) {
        ...
        RedisConnection conn = getConnection();
        try {
            bytes = conn.get(serializeKey(CommonConstants.REDIS_TOKEN_AUTH + token));
        } finally {
            conn.close();
        }
    ...
    private void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication, boolean isRenew) {
        ...
        byte[] authKey = serializeKey(CommonConstants.REDIS_TOKEN_AUTH + token.getValue());
        byte[] authToAccessKey = serializeKey(AUTH_TO_ACCESS + authenticationKeyGenerator.extractKey(authentication));
        byte[] approvalKey = serializeKey(CommonConstants.REDIS_UNAME_TO_ACCESS + getApprovalKey(authentication));
        byte[] clientId = serializeKey(CommonConstants.REDIS_CLIENT_ID_TO_ACCESS + authentication.getOAuth2Request().getClientId());
    ...
    public void removeAccessToken(String tokenValue) {
        ...
        byte[] authKey = serializeKey(CommonConstants.REDIS_TOKEN_AUTH + tokenValue);
        ...
        try {
            ...
            if (authentication != null) {
                ...
                byte[] unameKey = serializeKey(CommonConstants.REDIS_UNAME_TO_ACCESS + getApprovalKey(authentication));
                byte[] clientId = serializeKey(CommonConstants.REDIS_CLIENT_ID_TO_ACCESS + authentication.getOAuth2Request().getClientId());
                conn.openPipeline();
    ...
    public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
        byte[] approvalKey = serializeKey(CommonConstants.REDIS_UNAME_TO_ACCESS + getApprovalKey(clientId, userName));
        List<byte[]> byteList;
        RedisConnection conn = getConnection();
        try {
    ...
    public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
        byte[] key = serializeKey(CommonConstants.REDIS_CLIENT_ID_TO_ACCESS + clientId);
        List<byte[]> byteList;
        RedisConnection conn = getConnection();
        try {
